...
@interface CCAdDetailViewController ()<UIWebViewDelegate>
{
...
@implementation CCAdDetailViewController
-(instancetype)init{
    ...
    if (self) {
        ...
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:btn2];
            ...
            _titleLable=[[UILabel alloc]initWithFrame:CGRectMake(25.0/360.0*ScreenWidth, 10.0/667.0*ScreenHeight, 320.0/360.0*ScreenWidth, 50.0/667.0*ScreenHeight)];
...
-(void)requestData{
    ...
    [manager GET:url parameters:nil success:^(NSURLSessionTask *task, id responseObject) {
       // NSLog(@"广告分享内容＝＝JSON: %@", responseObject);
...
-(void)setModel:(CCAdDetailModel *)model{
    if (_model!=model) {
        ...
        if ([_model.video isEqualToString:@""]) {
            ...
            if ([_model.copyfrom isEqualToString:@""]) {
                ...
                _fromeLable.text=nil;
                _webView.frame =CGRectMake(0.0, 280.0/667.0*ScreenHeight, self.view.bounds.size.width, ScreenHeight-355.0/667.0*ScreenHeight);
