...
@interface CCSecListWebViewController ()<UIWebViewDelegate>
{
...
@implementation CCSecListWebViewController
...
-(void)setModel:(CCSecListModel *)model{
    if (_model!=model) {
        ...
        _fromeLable.text=[NSString stringWithFormat:@"%@",_model.copyfrom];
...
-(void)requestData{
    ...
    [manager GET:url parameters:nil success:^(NSURLSessionTask *task, id responseObject) {
       // NSLog(@"分享内容＝＝JSON: %@", responseObject);
