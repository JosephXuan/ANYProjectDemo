...
#define kNavigationBarTitleFont [[CCAppSeting shareInstance] systemBoldFontWithSize:18.f]
#import "AppDelegate.h"
@interface CCNavgationController ()<UIGestureRecognizerDelegate>
{
...
@implementation CCNavgationController
+ (instancetype)navigationWithWrapController:(UIViewController *)controller{
    return [[CCNavgationController alloc] initWithRootViewController:controller];
}
+ (void)initialize {
    if (self == [CCNavgationController class]) {
        UINavigationBar *naviBar = [UINavigationBar appearance];
        naviBar.tintColor = [CCAppSeting shareInstance].titleColor;
        naviBar.barTintColor = [CCAppSeting shareInstance].appMainColor;
        naviBar.titleTextAttributes = @{NSForegroundColorAttributeName: [CCAppSeting shareInstance].titleColor,NSFontAttributeName: [UIFont systemFontOfSize:18.f]};
        ...
        [naviBar setBackgroundImage:[UIImage gp_imageWithView:backgroud]  forBarMetrics:UIBarMetricsDefault];
        
        UIView *shadow = [[UIView alloc] initWithFrame:CGRectMake(.0f, .0f, ScreenWidth, kGlobalSeparatorHeight)];
        shadow.backgroundColor = [UIColor colorWithHexString:kGlobalSeparatorColorStr];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName : [CCAppSeting shareInstance].titleColor, NSFontAttributeName : [UIFont systemFontOfSize:18.f]};    [self.navigationBar setNavBarShadowColor:[UIColor colorWithHexString:kGlobalSeparatorColorStr] withAlpha:1.0f];
...
//self.leftVC = leftSortVC;
   // [self addChildViewController:self.leftVC];
       AppDelegate *tempAppDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    
    [self addChildViewController:tempAppDelegate.mainVC];
    self.mainVC = tempAppDelegate.mainVC;
