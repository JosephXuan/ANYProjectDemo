...
+ (instancetype)imageWithALogoWithSize:(CGSize)size
{
    ...
    {
        newSize = CGSizeMake(120, 80);
    }
    frame.size = newSize;
    UIView *backgroudV = [[UIView alloc] initWithFrame:frame];
    backgroudV.backgroundColor = [UIColor colorWithHexString:@"#dedede" alpha:1.0f];
    UIImage *img = [UIImage imageNamed:@"load_logo_icon"];
    UIImageView *imageView = [[UIImageView alloc] initWithImage:img];
    if ( MAX(img.size.width, img.size.height) > MIN(newSize.width, newSize.height) )
    {
        imageView.bounds = CGRectMake(0, 0, MIN(newSize.width, newSize.height), MIN(newSize.width, newSize.height));
    }
    [backgroudV addSubview:imageView];
    imageView.center = backgroudV.center;
    return [UIImage gp_imageWithView:backgroudV];
}
